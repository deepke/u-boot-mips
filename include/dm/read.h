...
static inline bool dev_read_bool(const struct udevice *dev,
				 const char *propname)
{
	while (!ofnode_valid(dev_ofnode(dev))) {
		dev = dev->parent;
	}
...
#define dev_for_each_subnode(subnode, dev) \
	for (subnode = (ofnode_valid(dev_ofnode(dev)))? dev_read_first_subnode(dev):(dev_ofnode(dev)); \
	     (ofnode_valid(dev_ofnode(dev))) && ofnode_valid(subnode); \
