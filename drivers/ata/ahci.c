...
# endif
{
	...
	if (vendor == PCI_VENDOR_ID_LOONGSON) 
		uc_priv->mmio_base = dm_pci_map_bar(dev, PCI_BASE_ADDRESS_0,
				PCI_REGION_MEM);
#else
	...
	if (vendor == PCI_VENDOR_ID_LOONGSON) 
		uc_priv->mmio_base = pci_map_bar(dev, PCI_BASE_ADDRESS_0,
				PCI_REGION_MEM);
#endif
#else
			...
			unsigned char *buf, int buf_len)
{
	...
	phys_addr_t pa = tobus(virt_to_phys(buf));
	u32 sg_count;
	int i;
...
static void ahci_fill_cmd_slot(struct ahci_ioports *pp, u32 opts)
{
	...
	pp->cmd_slot->tbl_addr = cpu_to_le32(lower_32_bits(tobus(pa)));
#ifdef CONFIG_PHYS_64BIT
	pp->cmd_slot->tbl_addr_hi = cpu_to_le32(upper_32_bits(tobus(pa)));
#endif
}
