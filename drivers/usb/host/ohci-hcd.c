	...
	case PIPE_INTERRUPT:
		...
		for (i = 0; i < ep_rev(6, interval); i += inter) {
			...
			for (ed_p = &(ohci->hcca->int_table[\
				...
				ed_p = &(((ed_t *)frombus(ed_p))->hwNextED))
					inter = ep_rev(6,
						 ((ed_t *)ed_p)->int_interval);
			ed->hwNextED = *(__u32*)frombus(ed_p);
			flush_dcache_ed(ed);
			*(__u32*)frombus(ed_p) = m32_swap((unsigned long)tobus(ed));
			    ...
			    unsigned index, unsigned period)
{
	...
	for (; index < NUM_INTS; index += period) {
		...
		while (*(__u32 *)frombus(ed_p) != 0) {
			if (((struct ed *)(uintptr_t)
					frombus(m32_swap((unsigned long)ed_p))) == ed) {
				*(__u32*)frombus(ed_p) = ed->hwNextED;
				aligned_ed_p = (unsigned long)ed_p;
				aligned_ed_p &= ~(ARCH_DMA_MINALIGN - 1);
				flush_dcache_range(frombus(aligned_ed_p),
					frombus(aligned_ed_p) + ARCH_DMA_MINALIGN);
				break;
			}
			ed_p = &(((struct ed *)
				     frombus(m32_swap((unsigned long)ed_p)))->hwNextED);
		}
	}
}
...
static int ep_unlink(ohci_t *ohci, ed_t *edi)
{
	...
	case PIPE_BULK:
		if (ed->ed_prev == NULL) {
			...
			ohci_writel(m32_swap(*((__u32 *)frombus(&ed->hwNextED))),
	...
	struct usb_device *dev, int index, urb_priv_t *urb_priv)
{
	...
	td = urb_priv->td [index] =
			     (td_t *)frombus(m32_swap(urb_priv->ed->hwTailP) & ~0xf);
	...
	td->hwCBP = m32_swap(tobus(data));
	if (data)
		td->hwBE = m32_swap(tobus(data + len - 1));
	else
		td->hwBE = 0;
...
static void dl_transfer_length(td_t *td)
{
	...
	if (!(usb_pipecontrol(lurb_priv->pipe) &&
		...
		if (tdBE != 0) {
			if (td->hwCBP == 0)
				lurb_priv->actual_length += tdBE - tobus(td->data) + 1;
			else
				lurb_priv->actual_length += tdCBP - tobus(td->data);
		}
	}
}
...
static td_t *dl_reverse_done_list(ohci_t *ohci)
{
	...
	while (td_list_hc) {
		td_list = (td_t *)frombus(td_list_hc);
