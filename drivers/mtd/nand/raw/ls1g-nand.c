...
static void nand_cache_inv(unsigned long base,unsigned long num)
{
    ...
    //flush_cache((base & 0x1fffffff)|0x80000000, num);
}
...
{
    *((volatile unsigned int *)0xbfe78018) = 0x300;
    info->num=0;
    info->size=0;
    info->cac_size = 0; 
    info->state = STATE_READY;
    ...
    info->nand_cs_rdy_map = 0x00024000;
    info->nand_cmd = 0;
...
#include "../../../board/ls1a/nand/nand_gpio.c"
...
*/
void ls1g_soc_nand_init(struct mtd_info *ls1g_soc_mtd)
{
 //       nand_gpio_read_id();
//        return 0;
        int val;
...
//       nand_test();    
	...
	//ls1g_soc_mtd = malloc(sizeof(struct mtd_info) + sizeof(struct nand_chip));
	//if (!ls1g_soc_mtd) {
...
//	memset(this, 0, sizeof(struct nand_chip));
	...
	info = malloc(sizeof(struct ls1g_nand_info));
	memset(info, 0, sizeof(struct ls1g_nand_info));
	/* Link the private data with the MTD structure */
	ls1g_soc_mtd->priv = info;
...
//      add_mtd_device(mtd_info,offset,size,name)
	ls1g_soc_mtd->name="nand0";
	printf("name:%s\n",ls1g_soc_mtd->name);
	add_mtd_device(ls1g_soc_mtd);
