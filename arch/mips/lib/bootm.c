...
static void linux_cmdline_append(bootm_headers_t *images)
{
	...
	/* append mem */
	mem = gd->ram_size >> 20;
#ifndef CONFIG_LINUX_DTB_BY_A2
	sprintf(buf, "mem=%luM", mem);
	linux_cmdline_set(buf, strlen(buf));
#endif

	/* append rd_start and rd_size */
	rd_start = (images->rd_start && !(images->rd_start &0x3fff))?images->rd_start:images->initrd_start;
	rd_size = images->initrd_end - images->initrd_start;

	if (rd_size) {
		sprintf(buf, "rd_start=0x%016llX", (long long)(long)rd_start);
		linux_cmdline_set(buf, strlen(buf));
...
#ifdef CONFIG_LINUX_DTB_BY_A2
	/*does not change memory info here*/
	return 0;
#else
...
static int boot_setup_fdt(bootm_headers_t *images)
{
	...
	if (rd_size) {
		bootargs = env_get("bootargs");
		args = malloc((bootargs?strlen(bootargs):0)+64);
		if (bootargs)
			sprintf(args, "%s rd_start=0x%016llX rd_size=0x%lX", bootargs, (long long)(long)rd_start, rd_size);
		else
			sprintf(args, "rd_start=0x%016llX rd_size=0x%lX", (long long)(long)rd_start, rd_size);
		env_set("bootargs", args);
	}
#endif

	rd_start = (images->rd_start && !(images->rd_start &0x3fff))?images->rd_start:images->initrd_start;
	rd_end = (images->rd_start && !(images->rd_start &0x3fff))?images->rd_end:images->initrd_end;

	images->initrd_start = virt_to_phys((void *)rd_start);
	images->initrd_end = virt_to_phys((void *)rd_end);
...
#ifdef CONFIG_BOARD_FIXUP_BEFORE_LINUX
...
static void boot_jump_linux(bootm_headers_t *images)
{
	...
	if (images->ft_len)
#ifdef CONFIG_LINUX_DTB_BY_A2
		kernel(linux_argc, (ulong)linux_argv, (ulong)images->ft_addr,
			linux_extra);
#else
...
#ifdef CONFIG_LINUX_DTB_BY_A2
		kernel(linux_argc, (ulong)linux_argv, (ulong)gd->fdt_blob,
			linux_extra);
#else
