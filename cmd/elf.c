...
static unsigned long do_bootelf_exec(ulong (*entry)(int, char * const[], char * const env[]),
				     int argc, char *const argv[])
{
	unsigned long ret;
	int i, j;

	static env_t	env_new;
	static char *env[MAXENV];
	ret = env_export(&env_new);
	for(i=0,j=0;i<MAXENV && env_new.data[j];i++)
	{
	 env[i] = env_new.data+j;
	 j+=strlen(env[i])+1;
	}
	env[i] = 0;
	/*
	...
	ret = entry(argc, argv, gd->fdt_blob);
#else
	ret = entry(argc, argv, env);
#endif
